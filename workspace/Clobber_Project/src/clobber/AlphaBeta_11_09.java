...
public class AlphaBeta_11_09 extends GamePlayer {
	...
	public GameMove getMove(GameState state, String lastMove) {
		ArrayList<ScoredClobberMove> allMoves = getMoves(state);
		
		int numPerThread = Math.max(1, (int)(allMoves.size() / AlphaBeta_11_09.MAX_THREADS));
		AlphaBetaThread[] threads = new AlphaBetaThread[allMoves.size() / numPerThread];
		
		for (int i = 0; i < threads.length; i++) {
			if (i == threads.length - 1) {
				List<ScoredClobberMove> moves = allMoves.subList(i * numPerThread, allMoves.size());
				threads[i] = new AlphaBetaThread((ClobberState)state, moves, depthLimit);
			}
			else {
				List<ScoredClobberMove> moves = allMoves.subList(i * numPerThread, (i + 1) * numPerThread);
				threads[i] = new AlphaBetaThread((ClobberState)state, moves, depthLimit);
			}
		...
		ScoredClobberMove best = threads[0].getClobberMove();
		
		for (int i = 1; i < threads.length; i++) {
			ScoredClobberMove temp = threads[i].getClobberMove();
	...
	private class AlphaBetaThread extends Thread {
		...
		public AlphaBetaThread(ClobberState state, List<ScoredClobberMove> moves, int maxDepth) {
			this.moves = moves;
			this.maxDepth = maxDepth;
			this.mvStack = new ScoredClobberMove[AlphaBeta_11_09.MAX_DEPTH];
		...
		public void run() {
			...
			for (int i = 0; i < moves.size(); i++) {
				...
				// Undo the move
				state.board[moves.get(i).row1][moves.get(i).col1] = 
						(state.getWho() == GameState.Who.HOME ? 
								ClobberState.homeSym : ClobberState.awaySym);
				
				state.board[moves.get(i).row2][moves.get(i).col2] = 
						(state.getWho() == GameState.Who.HOME ? 
								ClobberState.awaySym : ClobberState.homeSym);
			}
		}
